...
# Create storage account to access custom script extension file

resource "azurerm_storage_account" "SG1" {
  name                     = "wvmstorageaccount1"
  resource_group_name      = azurerm_resource_group.Tarun-VM1.name
  location                 = azurerm_resource_group.Tarun-VM1.location
...
resource "azurerm_storage_container" "CN1" {
  name                  = "container1"
  storage_account_name  = azurerm_storage_account.SG1.name
  container_access_type = "blob"

  depends_on = [
    azurerm_storage_account.SG1
  ]
}

# Here we are uploading our IIS configratiomscript as a blob to the Azure Storage account

resource "azurerm_storage_blob" "BL1" {
  name                   = "IIS_Config.ps1"
  storage_account_name   = azurerm_storage_account.SG1.name
  storage_container_name = azurerm_storage_container.CN1.name
  type                   = "Block"
  source                 = "IIS_Config.ps1"

  depends_on = [
    azurerm_storage_container.CN1
  ]
}
...
resource "azurerm_virtual_machine_extension" "VME1" {
  ...
  depends_on = [
    azurerm_storage_blob.BL1
  ]

  settings = <<SETTINGS
  {
    "commandToExecute": "Tarun-WVM1-Extension && uptime"
  }
  SETTINGS
